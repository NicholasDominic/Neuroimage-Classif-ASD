...
callbacks = [
    ModelCheckpoint(filepath="ckpt/checkpoint-augment-efficientnetb0-mask-{epoch:02d}-{val_acc:.3f}.h5", monitor="val_acc", save_best_only=True, mode="max"),
    TerminateOnNaN()
]
...
mask_EfficientNetB0_model.save("model/augment_mask_EfficientNetB0_model.h5")
mask_EfficientNetB0_model.save_weights("model/augment_mask_EfficientNetB0_weights.h5")
df.from_dict(mask_EfficientNetB0_model_result.history).to_csv('result/augment_mask_EfficientNetB0_model_result.csv', index=False)