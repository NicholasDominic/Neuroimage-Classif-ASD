...
callbacks = [
    ModelCheckpoint(filepath="ckpt/checkpoint-augment-efficientnetb0-epi-{epoch:02d}-{val_acc:.3f}.h5", monitor="val_acc", save_best_only=True, mode="max"),
    TerminateOnNaN()
]
...
epi_EfficientNetB0_model.save("model/augment_epi_EfficientNetB0_model.h5")
epi_EfficientNetB0_model.save_weights("model/augment_epi_EfficientNetB0_weights.h5")
df.from_dict(epi_EfficientNetB0_model_result.history).to_csv('result/augment_epi_EfficientNetB0_model_result.csv', index=False)